...
function run(withCustom) {
  ...
  for (var i = 0; i < 5; i++) {
    (function (i) {
      state.swatches.push({
        classList: { contains: function (c) { return c === 'az-selected' && state.sel === i; } },
        click: function () { state.sel = i; }
      });
    })(i);
  }
  // stub of the picker's callback: re-apply photo = deselect all presets
  var custom = withCustom ? function () { state.sel = -1; return true; } : null;
  var doc = { querySelectorAll: function () { return state.swatches; } };
...
function trace(withCustom, start, keys) {
  ...
  r.state.sel = start;
  var out = [];
  keys.split('').forEach(function (k) { r.cycle(k === 't' ? 1 : -1); out.push(r.state.sel); });
  return out;
}
