{
  ...
  channelSpaceSuffix =
    if channel == "stable"
    then ""
    else " ${lib.toUpper (lib.substring 0 1 channel)}${lib.substring 1 (-1) channel}";
  ...
  archive = assert lib.assertMsg (builtins.hasAttr stdenv.hostPlatform.system archives)
